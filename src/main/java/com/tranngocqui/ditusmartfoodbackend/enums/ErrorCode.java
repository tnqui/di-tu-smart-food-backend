...
public enum ErrorCode {
    ...
    PERMISSION_INVALID(1807, "Permission is invalid", HttpStatus.BAD_REQUEST),
    ...
    private static Map<String, ErrorCode> map() {
        return Arrays.stream(ErrorCode.values()).collect(Collectors.toMap(
                Enum::name, e -> e)
        );
    }

    public static ErrorCode parse(String messageCode) {
        ErrorCode e = map().get(messageCode);

        if (e == null) {
            throw new AppException(ErrorCode.WRONG_MESSAGE_CODE);
        }
